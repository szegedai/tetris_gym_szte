...
class Tetris():
    ...
    def __init__(self,
        ...
        self.pieces = [self.piece_dict[i] for i in pieces]

        for i in range(len(self.pieces)):
            self.pieces[i] = (np.array(self.pieces[i]) * (i + 1)).tolist()

        self.piece_colors = [self.piece_colors_dic[i] for i in (["B"] + pieces)]
    ...
    def render(self, mode="rgb_array"):
        ...
        img = img.resize((self.width * self.block_size, self.height * self.block_size), resample=PIL.Image.NEAREST)
        img = np.array(img)
        ...
        cv2.putText(img, "Score:", (self.width * self.block_size + int(self.block_size / 2), self.block_size),
                    fontFace=font_face, fontScale=font_scale, color=self.text_color, lineType=cv2.LINE_AA)
